-- =====================================================================
-- 001_init.sql – Initiales Schema Raumbuchung (MariaDB 10.11)
...
--
-- Bewusst NICHT enthalten (separate Scripts): Trigger, Prozeduren,
-- Sequenz meeting_seq, Seed-Daten, DB-Benutzer/Rechte.
-- =====================================================================
...
-- meeting_id bleibt über alle Versionen gleich (kein FK).
CREATE TABLE IF NOT EXISTS bookings (
  ...
  INDEX ix_bookings_room_time (room_id, starts_at, ends_at)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

-- Zwischentabelle: Einladung einer Person zu einem Meeting (über meeting_id, kein FK).
...
CREATE TABLE IF NOT EXISTS booking_participants (
  ...
  INDEX ix_booking_participants_meeting (meeting_id)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

-- ---------------------------------------------------------------------
-- 3. Beziehungen (Fremdschlüssel)
-- Standard RESTRICT: Stammdaten werden deaktiviert statt gelöscht.
-- Ausnahme laut ERM: password_reset_tokens -> users mit CASCADE.
-- ---------------------------------------------------------------------
...
ALTER TABLE bookings
  ...
  ADD CONSTRAINT fk_bookings_series FOREIGN KEY IF NOT EXISTS (booking_series_id)
    REFERENCES booking_series (booking_series_id) ON DELETE RESTRICT ON UPDATE RESTRICT,
...
-- booking_participants: users nimmt teil (Verbindung zu bookings nur über meeting_id)
